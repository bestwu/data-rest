...
public class DomainMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public DomainMethodArgumentResolver(RepositoryInvokerFactory invokerFactory, List<AbstractJackson2HttpMessageConverter> messageConverters) {
	...
	@Override public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
		Class<?> domainlType = parameter.getParameterType();
		Object domain = resolveModel(parameter, mavContainer, webRequest, binderFactory, domainlType);

		if (log.isDebugEnabled()) {
			log.debug("请求实体：" + StringUtil.valueOf(domain));
		}
	 ...
	 */
	protected Object resolveModel(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory, Class<?> domainType)
			throws Exception {
		Object content;
	 ...
	 * @throws Exception Exception
	 */
	private Object getObjectForUpdate(Class<?> domainType, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
		...
		Object objectForUpdate = invokerFactory.getInvokerFor(domainType).invokeFindOne(id);
		...
		webRequest.setAttribute(OLD_DOMAIN, oldModel, NativeWebRequest.SCOPE_REQUEST);
	 ...
	 * @throws Exception Exception
	 */
	private Object readObject(MethodParameter parameter, Object domain, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
		...
		if (readRequestBody) {
			...
			for (AbstractJackson2HttpMessageConverter messageConverter : messageConverters) {
				if (messageConverter.canRead(domainType, contentType)) {
					ObjectMapper mapper = messageConverter.getObjectMapper();
					mapper.readerForUpdating(domain).readValue(request.getBody());
					break;
				}
			}
		}

		WebDataBinder binder = binderFactory.createBinder(webRequest, domain, name);
		...
		return binder.convertIfNecessary(binder.getTarget(), domainType);
	}
	 ...
	 * @throws Exception Exception
	 */
	protected final Object createAttribute(String attributeName, Class<?> domainType,
			WebDataBinderFactory binderFactory, NativeWebRequest request) throws Exception {

		String value = getRequestValueForAttribute(attributeName, request);
		if (value != null) {
			Object attribute = createAttributeFromRequestValue(
					value, attributeName, domainType, binderFactory, request);
			if (attribute != null) {
				return attribute;
			}
		}
	...
	protected Object createAttributeFromRequestValue(String sourceValue, String attributeName,
			Class<?> domainType, WebDataBinderFactory binderFactory, NativeWebRequest request)
			throws Exception {
		...
		if (conversionService != null) {
			...
			TypeDescriptor target = TypeDescriptor.valueOf(domainType);
			if (conversionService.canConvert(source, target)) {
				return binder.convertIfNecessary(sourceValue, domainType);
			}
		}
		return null;
	}
